...
class S3Sync {
  ...
  async syncDirectory(localDir, s3Path) {
    try {
      ...
      const profileFlag = config.s3.profile
      ...
      this.log(
        `Syncing ${localDir} to ${s3Url} (size-only comparison for efficiency)...`
      );
  ...
  async syncOnlyChangedFiles(localDir, s3Path) {
    try {
      ...
      this.log(
        `Found ${changedFiles.length} recently changed files: ${changedFiles.join(', ')}`
      );
          ...
          if (stdout.includes('upload:')) {
            ...
            if (file.endsWith('.mp3')) {
              ...
              if (chapterId.match(/^\d+$/)) {
                ...
                this.log(
                  `✓ Updated published timestamp for chapter ${chapterId}`
                );
              }
            }
          }
  ...
  async syncPodcastFiles() {
    try {
      ...
      const quickResult = await this.syncOnlyChangedFiles(
        config.paths.public,
        config.s3.prefix
      );
      ...
      if (quickResult.uploadCount > 0) {
        this.log(
          `✓ Quick sync completed: ${quickResult.uploadCount} files uploaded`
        );

        // Also sync any new audio files
        this.log('🎧 Checking for new audio files...');
        const audioResult = await this.syncOnlyChangedFiles(
          config.paths.output,
          config.s3.prefix + 'audio/'
        );

        if (audioResult.uploadCount > 0) {
          this.log(
            `✓ Audio sync completed: ${audioResult.uploadCount} files uploaded`
          );
        }
      ...
      const fullResult = await this.syncDirectory(
        config.paths.public,
        config.s3.prefix
      );
      ...
      const mp3Files = audioFiles.filter(
        (file) => file.endsWith('.mp3') && !file.startsWith('segment_')
      );
      ...
      for (const mp3File of mp3Files) {
        ...
        try {
          ...
          this.log(
            `Warning: Failed to upload ${mp3File}: ${error.message}`,
            'warning'
          );
        }
      }
      ...
      this.log(
        `📡 Feed available at: https://www.porivo.com/${config.s3.prefix}feed.xml`
      );
    } catch (error) {
    ...
    case 'sync':
      ...
      sync
        ...
        .then(() => {
          ...
          console.log(
            `Feed URL: https://www.porivo.com/${config.s3.prefix}feed.xml`
          );
        })
        .catch(console.error);
      break;
