<script>
  ...
  let speakerViewMode = 'speakers'; // 'segments' or 'speakers'
  ...
  // Calculate speakers without voice assignments
  $: speakersWithoutVoice = speakers.filter((s) => !s.voiceId).length;
  ...
  async function loadChapterText() {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/text`
      );
      if (!response.ok) {
  ...
  async function scrapeChapterContent() {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/scrape`,
        {
          method: 'POST',
        }
      );
  ...
  async function loadSpeakers() {
    ...
    try {
      ...
      const [segmentsResponse, speakersResponse, voicesData] =
        await Promise.all([
      ...
      if (!segmentsResponse.ok)
        throw new Error('Failed to load chapter segments');
      ...
      allSegments = segments
        .map((segmentData) => {
          ...
          return {
            ...
            isUnmatched:
              !segment.speaker_name || segment.speaker_name === 'Unknown',
          };
        })
  ...
  async function checkMp3Exists() {
    try {
      const response = await fetch(
        `${API_URL}/api/audio/${chapter.id}/${chapter.id}.mp3`,
        {
          method: 'HEAD',
        }
      );
  ...
  async function updateSegmentSpeaker(segmentIndex, newSpeakerName) {
    try {
      ...
      if (segment) {
        ...
        const segmentsToUpdate = allSegments.filter(
          (seg) => seg.speakerName === oldSpeakerName
        );
        ...
        console.log(
          `Updated speaker for ${segmentsToUpdate.length} segments from "${oldSpeakerName}" to "${newSpeakerName}"`
        );
      }
    } catch (err) {
  ...
  async function updateSegmentSpeakerOnServer(segmentIndex, newSpeakerId) {
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/segments/${segmentIndex}/speaker`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update segment speaker: ${response.statusText}`
        );
      }
  ...
  async function assignVoice(speakerId, voiceId, voiceName) {
    try {
      const response = await fetch(
        `${API_URL}/api/speakers/${speakerId}/voice`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function identifySpeakers() {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/identify-speakers`,
        {
          method: 'POST',
        }
      );
  ...
  async function markSpeakerIdComplete() {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/mark-speakers-complete`,
        {
          method: 'POST',
        }
      );
  ...
  async function regenerateSegment(segmentIndex) {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/segments/${segmentIndex}/regenerate`,
        {
          method: 'POST',
        }
      );
  ...
  async function rebuildChapter() {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/rebuild`,
        {
          method: 'POST',
        }
      );
  ...
  async function debugMergeChapter() {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/debug-merge`,
        {
          method: 'POST',
        }
      );
  ...
  async function pollRebuildProgress() {
    try {
      const response = await fetch(
        `${API_URL}/api/chapters/${chapter.id}/rebuild-progress`
      );
  ...
  async function startBuild(forceRebuild = false) {
    ...
    buildProgress = {
      ...
      message: forceRebuild
        ? 'Force rebuilding chapter MP3...'
        : 'Building chapter MP3...',
    };
        ...
        <div class="editor-container">
          {#if !content && !scraping}
            <div class="no-content-state">
              ...
              <p>
                This chapter is a stub record with title and URL, but no content
              ...
              {#if speakerIdProgress.totalChunks > 1}
                <div class="chunk-progress">
                  <div class="progress-bar">
                    <div
                      class="progress-fill"
                      style="width: {((speakerIdProgress.currentChunk + 1) /
                        speakerIdProgress.totalChunks) *
                        100}%"
                    ></div>
                  </div>
                ...
                <div class="progress-stats">
                  <div class="stat-item">
                    ...
                    <span class="stat-value"
                      >{speakerIdProgress.contentLength.toLocaleString()} characters</span
                    >
                  </div>
                  {#if speakerIdProgress.knownSpeakers !== undefined}
                    <div class="stat-item">
                      ...
                      <span class="stat-value"
                        >{speakerIdProgress.knownSpeakers}</span
                      >
                    </div>
                  {/if}
                </div>
                ...
                <div class="progress-results">
                  ...
                  <div class="results-stats">
                    <div class="stat-item">
                      ...
                      <span class="stat-value"
                        >{speakerIdProgress.segments}</span
                      >
                    </div>
                  </div>
          ...
          {:else}
            <div class="speakers-header-actions">
              <div class="view-toggle">
                ...
                >
                  👥 Speakers View
                  {#if speakersWithoutVoice > 0}
                    <span class="badge warning">{speakersWithoutVoice}</span>
                  {/if}
                </button>
                <button
                  class="toggle-button"
              ...
              <div class="segments-view">
                ...
                <div class="segments-list">
                  {#each allSegments as segment}
                    <div
                      class="segment-row"
                      class:unmatched={segment.isUnmatched}
                    >
                    ...
                    <div class="voice-controls">
                      ...
                      {#if speaker.voiceId}
                        <button
                          ...
                          on:click={() =>
                            previewVoice(speaker.voiceId, speaker.name)}
                          disabled={previewingVoices.has(
                  ...
                  <div class="speaker-segments">
                    ...
                    {#each speaker.segments.slice(0, 3) as segment, segmentIndex}
                      <div class="segment-item">
                        <div class="segment-type {segment.type}">
                          {segment.type}
                        </div>
                        ...
                        <button
                          ...
                          disabled={!speaker.voiceId ||
                            previewingSegments.has(
                              `segment-${speaker.voiceId}-${segment.index}`
                            )}
                          title={speaker.voiceId
                            ? 'Preview this segment with assigned voice'
                            : 'Assign a voice first to preview'}
                        >
          ...
          {:else}
            <div class="tts-info">
              <div class="tts-summary">
                ...
                <div class="tts-stats">
                  ...
                  {#if ttsInfo?.fileSize}
                    <div class="stat-item">
                      ...
                      <div class="stat-value">
                        {(ttsInfo.fileSize / 1024 / 1024).toFixed(1)}MB
                      </div>
                    </div>
                  {/if}
                  ...
                  {#if ttsInfo?.totalSegments}
                    <div class="stat-item">
                      ...
                      <div class="stat-value">
                        {ttsInfo.cachedSegments} / {ttsInfo.totalSegments} cached
                      </div>
                    </div>
                  {/if}
                </div>
                ...
                <div
                  style="background: #f0f0f0; padding: 8px; margin: 8px 0; font-family: monospace; font-size: 12px;"
                >
                  DEBUG: cachedSegments.length = {cachedSegments.length}
                  {#if cachedSegments.length > 0}
                    <br />First segment: {JSON.stringify(
                      cachedSegments[0]?.segment?.text?.substring(0, 50) ||
                        'undefined'
                    )}
                  {/if}
                </div>
                      ...
                      {#each cachedSegments as segmentFile, index}
                        <div class="segment-cache-item">
                          <div class="segment-info">
                            ...
                            <div class="segment-details">
                              ...
                              <div class="segment-meta">
                                Speaker: {segmentFile.segment.speaker_name} • Type:
                                {segmentFile.segment.type} •
                          ...
                          <div class="segment-actions">
                            <div
                              class="segment-status {segmentFile.exists
                                ? 'cached'
                                : 'missing'}"
                            >
                              {#if segmentFile.exists}
              ...
              <div class="build-summary">
                ...
                <div class="build-stats">
                  <div class="stat-item">
                    ...
                    <div
                      class="stat-value {buildInfo.mp3Exists
                        ? 'exists'
                        : 'missing'}"
                    >
                  ...
                  {#if buildInfo.missingSegments > 0}
                    <div class="stat-item">
                      ...
                      <div class="stat-value error">
                        {buildInfo.missingSegments} segments
                      </div>
                    </div>
                  {/if}
                  ...
                  {#if buildInfo.audioDuration}
                    <div class="stat-item">
                      <div class="stat-label">Duration:</div>
                      <div class="stat-value">
                        {Math.round(buildInfo.audioDuration)}s
                      </div>
                    </div>
                  {/if}
                  ...
                  {#if buildInfo.audioFileSize}
                    <div class="stat-item">
                      <div class="stat-label">File Size:</div>
                      <div class="stat-value">
                        {(buildInfo.audioFileSize / 1024 / 1024).toFixed(1)}MB
                      </div>
                    </div>
                  {/if}
                </div>
                ...
                <div class="rebuild-progress">
                        ...
                        {#each rebuildProgress.segments as segment}
                          <div class="segment-info">
                            ...
                            <span class="segment-duration"
                              >{segment.duration.toFixed(2)}s</span
                            >
                            <span class="segment-size"
                              >{(segment.size / 1024).toFixed(1)}KB</span
                            >
                          </div>
                        {/each}
                      </div>
                  ...
                  {#if rebuildProgress.status === 'completed'}
                    <div class="rebuild-complete">
                      ✅ Total Duration: {rebuildProgress.totalDuration
                        ? Math.round(rebuildProgress.totalDuration) + 's'
                        : 'N/A'}
                      | File Size: {rebuildProgress.fileSize
                        ? (rebuildProgress.fileSize / 1024 / 1024).toFixed(1) +
                          'MB'
                        : 'N/A'}
                    </div>
                  {/if}
                    ...
                    {#each buildInfo.segments as segment}
                      <div class="segment-file" class:missing={!segment.exists}>
                        <span class="segment-name"
                          >segment_{segment.index
                            .toString()
                            .padStart(3, '0')}.mp3</span
                        >
              ...
              <div class="publish-summary">
                ...
                <div class="publish-stats">
                  <div class="stat-item">
                    <div class="stat-label">Status:</div>
                    <div
                      class="stat-value {publishInfo?.isPublished
                        ? 'published'
                        : 'unpublished'}"
                    >
                  ...
                  {#if publishInfo?.publishedAt}
                    <div class="stat-item">
                      ...
                      <div class="stat-value">
                        {formatDate(publishInfo.publishedAt)}
                      </div>
                    </div>
                  {/if}
                  ...
                  <div class="stat-item">
                    ...
                    <div class="stat-value">
                      {chapter.fileSize
                        ? (chapter.fileSize / 1024 / 1024).toFixed(1) + 'MB'
                        : '-'}
                    </div>
                  </div>
                </div>
              ...
              <div class="feed-info">
                ...
                <div class="feed-details">
                  <div class="feed-item">
                    ...
                    <a
                      href="https://www.porivo.com/podcasts/feed.xml"
                      target="_blank"
                    >
                  ...
                  <div class="feed-item">
                    ...
                    <a
                      href="https://www.porivo.com/podcasts/audio/{chapter.id}.mp3"
                      target="_blank"
                    >
                ...
                <div class="subscribe-info">
                  ...
                  <ol>
                    ...
                    <li>
                      Paste: <code
                        >https://www.porivo.com/podcasts/feed.xml</code
                      >
                    </li>
                    <li>Tap "Subscribe"</li>
                  </ol>
