...
class APIServer {
  ...
  setupRoutes() {
      ...
      try {
        ...
        try {
          ...
          return {
            ...
            message:
              'This chapter is a stub. Content needs to be scraped first.',
            isStub: true,
          };
        }
    ...
    this.fastify.delete('/api/chapters/:id', async (request, reply) => {
      try {
        ...
        this.log(
          `Failed to delete chapter ${request.params.id}: ${error.message}`,
          'error'
        );
        reply.code(500);
        return { error: error.message };
    ...
    this.fastify.post('/api/jobs/sync', async (request, reply) => {
      try {
        ...
        return {
          success: true,
          message: 'Stage 1: Chapter extraction started',
        };
      } catch (error) {
        reply.code(500);
        return { error: error.message };
    ...
    this.fastify.post(
      ...
      async (request, reply) => {
        try {
          ...
          this.log(
            `Current highest chapter: ${highestChapterNumber}, searching for: ${nextChapterNumber}`
          );

          // For bulk extraction, we need to wait for completion
          const waitForCompletion = request.body.waitForCompletion || false;

          if (waitForCompletion) {
            // Synchronous version for bulk extraction
            try {
              const result =
                await this.processSearchAndLoadNextChapter(nextChapterNumber);
              if (result && result.id) {
                return {
                  success: true,
                  message: `Successfully loaded Chapter ${nextChapterNumber}`,
                  chapterId: result.id,
                ...
                return {
                  success: false,
                  error: `Chapter ${nextChapterNumber} not found or failed to load`,
                  chapterNumber: nextChapterNumber,
                };
              }
            } catch (error) {
            ...
            // Async version for single loads
    ...
    this.fastify.post('/api/jobs/identify-speakers', async (request, reply) => {
      try {
        this.log(
          '🎭 Stage 2: Starting speaker identification for all chapters...'
        );
        ...
        this.log(
          `Found ${chapters.length} chapters for speaker identification`
        );
        ...
        for (const chapter of chapters) {
          try {
            ...
            this.log(
              `❌ Failed speaker identification for ${chapter.title}: ${error.message}`,
              'error'
            );
          }
        }
        ...
        this.log(
          `🎭 Stage 2 complete: ${processed}/${chapters.length} chapters processed`
        );
        return {
          success: true,
    ...
    this.fastify.post(
      '/api/chapters/:id/identify-speakers',
      async (request, reply) => {
        try {
          const chapterId = request.params.id;
          ...
          this.log(
            `Speaker identification failed for ${request.params.id}: ${error.message}`,
            'error'
          );
          reply.code(500);
          return { error: error.message };
    ...
    this.fastify.get(
      '/api/chapters/:id/speaker-id-progress',
      async (request, reply) => {
        try {
          const chapterId = request.params.id;
    ...
    this.fastify.post(
      '/api/chapters/:id/mark-speakers-complete',
      async (request, reply) => {
        try {
          const chapterId = request.params.id;
          this.log(
            `✅ Marking speaker identification as complete for chapter ${chapterId}`
          );
    ...
    this.fastify.post(
      '/api/chapters/:id/generate-audio',
      async (request, reply) => {
        try {
          const chapterId = request.params.id;
          ...
          this.log(
            `Audio generation failed for ${request.params.id}: ${error.message}`,
            'error'
          );
          reply.code(500);
          return { error: error.message };
    ...
    this.fastify.get(
      '/api/chapters/:id/generation-progress',
      async (request, reply) => {
        try {
          const chapterId = request.params.id;
    ...
    this.fastify.put(
      '/api/chapters/:id/segments/:index/speaker',
      async (request, reply) => {
        try {
          const chapterId = request.params.id;
          ...
          this.log(
            `👤 Updating speaker for segment ${segmentIndex} in chapter ${chapterId} to speaker ${speakerId}`
          );

          // Update the speaker assignment in the database
          await this.db.updateSegmentSpeaker(
            chapterId,
            segmentIndex,
            speakerId
          );
          ...
          const chapterMp3Path = path.join(
            config.paths.output,
            `${chapterId}.mp3`
          );
          try {
            ...
            this.log(
              `🗑️ Deleted chapter MP3 for ${chapterId} after speaker update`
            );
          ...
          this.log(
            `❌ Failed to update segment speaker: ${error.message}`,
            'error'
          );
          reply.code(500);
          return { error: error.message };
    ...
    this.fastify.post(
      '/api/chapters/:id/segments/:index/regenerate',
      async (request, reply) => {
        try {
          const chapterId = request.params.id;
          const segmentIndex = parseInt(request.params.index);
          ...
          const segmentFile = await worker.regenerateSegment(
            chapterId,
            segmentIndex
          );
          await worker.close();
          ...
          try {
            await fs.unlink(chapterMp3Path);
            this.log(
              `🗑️ Deleted chapter MP3 for ${chapterId} after segment regeneration`
            );
    ...
    this.fastify.get(
      '/api/chapters/:id/rebuild-progress',
      async (request, reply) => {
        const chapterId = request.params.id;
        ...
        if (
          ...
          Date.now() - progress.startTime > 5 * 60 * 1000
        ) {
        ...
        try {
          ...
          if (filename.startsWith('segment_')) {
            ...
            return reply.sendFile(
              filename,
              path.join(config.paths.output, 'segments', chapterId)
            );
          } else {
    ...
    this.fastify.post('/api/sync/s3', async (request, reply) => {
      try {
        ...
        this.log(
          `📡 Feed available at: https://www.porivo.com/podcasts/feed.xml`
        );
    ...
    this.fastify.put('/api/speakers/:id/voice', async (request, reply) => {
      try {
        ...
        this.log(
          `Updated voice for speaker ${speakerId}: ${voiceName} (${voiceId})`
        );
    ...
    this.fastify.get(
      '/api/speakers/:id/affected-chapters',
      async (request, reply) => {
        try {
          const speakerId = request.params.id;
          const chapters =
            await this.db.getChaptersAffectedByVoiceChange(speakerId);
          return chapters;
        } catch (error) {
          reply.code(500);
          return { error: error.message };
    ...
    this.fastify.delete('/api/speakers/:id', async (request, reply) => {
      try {
        ...
        if (segmentCount > 0 && !force) {
          ...
          return {
            error:
              'Cannot delete character with existing segments. Use force=true to delete anyway, or use merge to combine with another character.',
            segmentCount,
          };
        }
        ...
        this.log(
          `Deleted character with ID ${speakerId}${force ? ' (forced)' : ''}`
        );
    ...
    this.fastify.post('/api/voices/preview', async (request, reply) => {
      try {
        ...
        if (!voice || voice.provider === 'openai') {
          ...
          const response = await fetch(
            `https://api.elevenlabs.io/v1/text-to-speech/${voice.settings.voice_id}`,
            {
              method: 'POST',
              headers: {
          ...
          throw new Error(
            `Voice provider ${voice.provider} not yet implemented`
          );
        }
      } catch (error) {
        reply.code(500);
      ...
      async (request, reply) => {
        try {
          ...
          if (!config.elevenlabs?.apiKey) {
            ...
            return {
              error:
                'ElevenLabs API key not configured. Please set ELEVENLABS_API_KEY in your .env file',
            };
          }
          ...
          if (!response.ok) {
            ...
            if (response.status === 404 || response.status === 400) {
              ...
              if (
                ...
                this.log(
                  `Voice not found in user library, trying Voice Library API: ${voiceId}`
                );

                // Use the correct Voice Library search endpoint
                try {
                  this.log(
                    `Searching Voice Library with search parameter: ${voiceId}`
                  );
                  ...
                  if (libraryResponse.ok) {
                    ...
                    if (libraryData.voices && libraryData.voices.length > 0) {
                      ...
                      this.log(
                        `Found Voice Library voice: ${voice.name} (${voice.voice_id})`
                      );
                      ...
                      return {
                        ...
                        tags: [
                          ...
                          voice.category,
                          ...(voice.descriptive
                            ? voice.descriptive.split(',').map((s) => s.trim())
                            : []),
                        ]
                          .filter(Boolean)
                          .join(', '),
                      ...
                      this.log(
                        `No voices found in Voice Library search for: ${voiceId}`
                      );
                    }
                  } else {
                    const errorText = await libraryResponse.text();
                    this.log(
                      `Voice Library search failed: ${libraryResponse.status} ${errorText}`,
                      'error'
                    );
                  }
                } catch (libraryError) {
                  this.log(
                    `Failed to search Voice Library: ${libraryError.message}`,
                    'error'
                  );
                }
                ...
                this.log(
                  `Voice not found in Voice Library either, using placeholder: ${voiceId}`
                );
                return {
            ...
            if (response.status === 401) {
              ...
              this.log(
                `ElevenLabs API authentication failed: ${JSON.stringify(errorData)}`,
                'error'
              );
              ...
              if (errorData.detail?.message?.includes('missing_permissions')) {
                ...
                return {
                  error:
                    'Your ElevenLabs API key is missing the "voices_read" permission. Please update your API key permissions at elevenlabs.io',
              ...
              return {
                error:
                  'ElevenLabs API authentication failed. Please check your ELEVENLABS_API_KEY in .env file',
              };
            }
  ...
  async processLoadNextChapter(postId) {
    try {
      ...
      if (chapterData) {
        ...
        this.log(
          `📝 Content length: ${chapterData.content?.length || 0} characters`
        );
        ...
        this.log(
          `🎭 Automatically starting speaker identification for loaded chapter...`
        );
        try {
          await this.processChapterSpeakerIdentification(chapterData.id);
          this.log(
            `✅ Speaker identification completed for: ${chapterData.title}`
          );
        } catch (error) {
          this.log(
            `⚠️ Speaker identification failed: ${error.message}`,
            'warning'
          );
        }
      ...
      this.log(
        `❌ Failed to load chapter ${postId}: ${error.message}`,
        'error'
      );
      throw error;
    }
  ...
  async processScrapeChapterContent(chapterId, chapterUrl) {
    try {
      this.log(
        `📖 Scraping content for chapter ${chapterId} from: ${chapterUrl}`
      );
      ...
      if (chapterData) {
        ...
        this.log(
          `🎭 Automatically starting speaker identification for chapter ${chapterId}...`
        );
        try {
          await this.processChapterSpeakerIdentification(chapterId);
          this.log(
            `✅ Speaker identification completed for chapter ${chapterId}`
          );
        } catch (speakerError) {
          this.log(
            `⚠️ Speaker identification failed for chapter ${chapterId}: ${speakerError.message}`,
            'warning'
          );
      ...
      this.log(
        `❌ Failed to scrape content for chapter ${chapterId}: ${error.message}`,
        'error'
      );
      throw error;
    }
  ...
  async processSearchAndLoadNextChapter(chapterNumber) {
    ...
    try {
      ...
      if (chapterInfo) {
        ...
        this.log(
          `🆔 Post ID: ${chapterInfo.url.match(/\/posts\/.*-(\d+)$/)?.[1] || 'unknown'}`
        );

        // Extract post ID from URL
        const postIdMatch = chapterInfo.url.match(/\/posts\/.*-(\d+)$/);
        const postId = postIdMatch
          ? postIdMatch[1]
        ...
        if (chapterData) {
          ...
          // Automatically start speaker identification AFTER file is saved
          this.log(
            `🎭 Automatically starting speaker identification for found chapter...`
          );
          try {
            await this.processChapterSpeakerIdentification(chapterData.id);
            this.log(
          ...
          this.log(
            `⚠️ Found chapter but failed to scrape content. Stub record created.`
          );
          return stubData;
        }
      } else {
        ...
        this.log(
          `💡 It might not be published yet, might be from a different series, or use different naming`
        );
        return null;
      }
    } catch (error) {
      this.log(
        `❌ Failed to search and load Chapter ${chapterNumber}: ${error.message}`,
        'error'
      );
      throw error;
    } finally {
      ...
      if (scraper && scraper.browser) {
        try {
          ...
          this.log(`⚠️ Error closing scraper: ${closeError.message}`);
        }
      }
    }
  }
  ...
  async processChapterSpeakerIdentification(chapterId) {
    ...
    try {
      ...
      this.log(
        `📝 Analyzing chapter content (${chapterData.content.length} characters)...`
      );
      ...
      segments.forEach((segment) => {
        speakerCounts[segment.speaker] =
          (speakerCounts[segment.speaker] || 0) + 1;
      });
      ...
      this.log(
        `❌ Speaker identification failed for chapter ${chapterId}: ${error.message}`,
        'error'
      );
  ...
  async processMultiVoiceTTS() {
    try {
      ...
      this.log(
        `❌ Multi-voice TTS processing failed: ${error.message}`,
        'error'
      );
    }
  }
  ...
  async processChapterAudio(chapterId) {
    try {
      ...
      this.log(
        `❌ Audio generation failed for chapter ${chapterId}: ${error.message}`,
        'error'
      );
    }
  }
