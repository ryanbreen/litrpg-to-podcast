...
class PatreonScraper {
  ...
  async waitWithJitter() {
    const delay = config.scraping.delayBaseMs + 
  ...
  async retryWithBackoff(fn, retries = config.scraping.maxRetries) {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        this.log(`Retry ${i + 1}/${retries} in ${backoffDelay}ms: ${error.message}`);
  ...
  async getChapterLinks() {
    ...
    console.log(`🔍 Looking for posts matching pattern: /${config.patreon.chapterRegex}/i`);
    
    // Debug what's actually on the page
    const pageInfo = await this.page.evaluate(() => {
      const allLinks = Array.from(document.querySelectorAll('a'));
      const postLinks = Array.from(document.querySelectorAll('a[href*="/posts/"]'));
      const patreonLinks = allLinks.filter(link => link.href && link.href.includes('patreon.com'));
    ...
    if (allLinks.length > 0) {
      ...
      allLinks.slice(0, 5).forEach((link, i) => {
        console.log(`   ${i+1}. "${link.title}" -> ${link.url} (ID: ${link.id})`);
      });
    ...
    if (config.patreon.startFromChapter) {
      console.log(`🔽 Filtering chapters starting from chapter ${config.patreon.startFromChapter}`);
      ...
      console.log(`📖 After filtering: ${filteredChapters.length} chapters (${chapters.length - filteredChapters.length} skipped)`);
  ...
  async scrapeChapterContent(chapterUrl) {
    ...
    if (!this.page || !this.browser) {
      ...
      if (!this.page) {
        throw new Error('Failed to initialize browser page for content scraping');
      }
    }
  ...
  cleanContent(content) {
    ...
    cleaned = cleaned.replace(/^\d+\s+(second|minute|hour|day|week|month|year)s?\s+ago\s*$/gm, '');
    
    // Remove "just now" type indicators
    cleaned = cleaned.replace(/^just now\s*$/gmi, '');
    
    // Remove standalone dates that are on their own line
    cleaned = cleaned.replace(/^(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+\d{4}\s*$/gm, '');
  ...
  async scrapeChapterByUrl(url) {
    try {
      ...
      if (!this.page) {
        throw new Error('Failed to initialize browser page - page is null after init');
      }
      
      if (!this.browser) {
        throw new Error('Failed to initialize browser context - browser is null after init');
      }
  ...
  async scrapeCreatorPage() {
    ...
    const loginCheck = await this.page.evaluate(() => {
      ...
      const hasLogoutButton = document.querySelector('a[href*="/settings"]') !== null || 
                             document.querySelector('[data-tag="profile-button"]') !== null;
      const cookies = document.cookie;
      const hasSessionCookie = cookies.includes('session_id') || cookies.includes('patreon_device_id');
    ...
    this.log(`🔐 Login status: ${loginCheck.isLoggedIn ? 'Logged in' : 'NOT logged in'}`);
  ...
  async findNextChapter(currentChapterNumber) {
    ...
    try {
      ...
      const postLinks = await this.page.$$eval('[data-tag="post-card"]', 
        (cards, searchText) => {
          const results = [];
      ...
      if (postLinks.length > 0) {
        this.log(`✅ Found ${postLinks.length} matching posts in search results`);
